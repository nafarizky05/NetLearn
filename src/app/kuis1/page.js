...
export default function KuisModul1() {
  ...
  // Fungsi memuat data soal (diacak) & memvalidasi jumlah limit percobaan siswa
  const inisialisasiKuis = async (idSiswa) => {
    try {
      // 1. Cek berapa kali siswa sudah mengerjakan kuis modul 1
      ...
      console.error("Gagal memuat sistem kuis:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Logika kalkulasi nilai, feedback konstruktif, dan pengiriman rekam medis kuis ke Supabase
// Logika kalkulasi nilai, feedback konstruktif, dan pengiriman rekam medis kuis ke Supabase
  const handleSubmitKuis = async () => {
    ...
    // 1. Catat ke histori pengerjaan kuis (kuis_attempts)
    ...
    // 2. UPDATE KE USER_PROGRESS (Kolom lama aman terisi)
    await supabase
    .from('user_progress')
    .update({ skor_modul1: skorAkhir })
    .eq('id', siswaId);
    ...
    .upsert({
        siswa_id: siswaId,
        modul_id: 1, // Berdasarkan ID master_modul
  ...
  // Fungsi bantu untuk memberikan feedback personal yang memotivasi siswa
  const dapatkanFeedbackDinamis = (skor) => {
    if (skor >= 80)
      return {
        warna: "text-green-700 bg-green-50 border-green-200",
        teks: "🏆 Kerja bagus luar biasa! Kamu sudah memahami seluruh esensi dasar jaringan komputer dengan matang. Pertahankan performa hebatmu!",
      };
    if (skor >= 70)
      return {
        warna: "text-blue-700 bg-blue-50 border-blue-200",
        teks: "👍 Pemahamanmu sudah cukup baik dan kamu berhasil lulus batas minimal kuota nilai kuis. Sedikit review materi topologi akan menyempurnakan ilmumu!",
      };
    return {
      warna: "text-red-700 bg-red-50 border-red-200",
      teks: "📚 Masih ada beberapa konsep abstrak yang perlu diperkuat kembali. Jangan patah semangat! Silakan pelajari ulang materi modul dan manfaatkan kesempatan mencoba ulang kuis.",
    };
  };
      ...
      <main className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-6 rounded-2xl shadow-md border border-gray-100 max-w-md w-full text-center">
          ...
          <p className="text-sm text-gray-500 mt-2 leading-relaxed">
            ...
            untuk Kuis Modul 1. Tombol pengerjaan telah dikunci sistem secara
  ...
  const progressPersen = ((currentIndex + 1) / soalList.length) * 100;
        ...
        <section className="space-y-5 animate-fadeIn">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="grid gap-2.5 mt-2">
              {soalAktif.pilihan_opsi.map((opsi, i) => {
